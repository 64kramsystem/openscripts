...
- generic
  - `asm_listing_pdf`: renders highlighted x86 assembly listings with clickable references as PDF
  ...
  - `notif`: send a desktop notification; cross-platform (uses `notify-send` on Linux, `osascript` on Mac)
...
- system
  ...
  - `upgrade_bluez_from_source`: upgrade BlueZ from source, or remove a matching unmanaged source
    installation with `--uninstall-only` before migrating back to distro packages
